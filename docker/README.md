...
There's also `scripts/ci-helpers/build_instance.sh`, which can be used to build the *full* (main application image) like also *postgrest-rewriter* targets. To use this script please execute: `scripts/ci-helpers/build_instance.sh <image_tag> <src_dir> <registry_url>`. All parameters are obligatory.

Using `build_instance.sh` script to build the *full* target will result in a Docker image with properly set labels. Otherwise most of the labels will be set to `[unknown]`.
