...
CREATE OR REPLACE FUNCTION hafbe_endpoints.get_witness(_account TEXT,_limit INT = 1, _offset INT = 0, _order_by TEXT = 'votes', _order_is TEXT = 'desc')
RETURNS JSON
LANGUAGE 'plpgsql'
AS
$$
BEGIN
  ...
  END IF;
    ...
    RETURN CASE WHEN arr IS NOT NULL THEN to_json(arr) ELSE '[]'::JSON END FROM (
      SELECT ARRAY(
        SELECT hafbe_backend.get_setof_witness(_account, _limit, _offset, _order_by, _order_is)
      ) arr
    ) result;
END
$$
