...
CREATE OR REPLACE FUNCTION hafbe_backend.blocksearch_by_account_and_key_desc(
    _operation INT,
    _account TEXT,
    _from INT, 
    _to INT,
...
BEGIN  
  _count := hafbe_backend.blocksearch_by_account_and_key_desc_count(_operations, _account, _from, _to, _limit, _key_content, _setof_keys);
  ...
  FROM (
    ...
    SELECT 
      so.block_num,
      array_agg(DISTINCT so.op_type_id) AS op_type_ids
	  FROM source_ops so
	  JOIN filter_by_key fbk on so.operation_id = fbk.id
    GROUP BY so.block_num
    ORDER BY so.block_num DESC
    OFFSET _offset
...
CREATE OR REPLACE FUNCTION hafbe_backend.blocksearch_by_account_and_key_asc(
    _operation INT,
    _account TEXT,
    _from INT, 
    _to INT,
...
DECLARE
  ...
  _path3 TEXT[] := ARRAY(SELECT json_array_elements_text(_setof_keys->2));
BEGIN  
  _count := hafbe_backend.blocksearch_by_account_and_key_asc_count(_operations, _account, _from, _to, _limit, _key_content, _setof_keys);
