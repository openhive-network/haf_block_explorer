...
CREATE OR REPLACE FUNCTION hafbe_backend.blocksearch_desc(
    _operations INT[],
    _page INT,
    _limit INT
)
...
--plan is more stable with force_generic_plan
...
BEGIN
  _count := hafbe_backend.blocksearch_desc_count(_operations, NULL, NULL, _limit);
...
CREATE OR REPLACE FUNCTION hafbe_backend.blocksearch_asc(
    _operations INT[],
    _page INT,
    _limit INT
)
...
DECLARE 
  ...
  _offset INT := (((_page - 1) * _limit));
BEGIN
  _count := hafbe_backend.blocksearch_asc_count(_operations, NULL, NULL, _limit);
