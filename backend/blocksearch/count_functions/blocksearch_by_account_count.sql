...
BEGIN
  RETURN (
    ...
    (
      ...
      WHERE 
        aov.op_type_id = ANY((SELECT op_type_ids FROM ops_list)::SMALLINT[]) AND
        aov.account_id = (SELECT id FROM get_account_id) AND
        ((_from IS NULL) OR aov.block_num <= _to) AND
        ((_to IS NULL) OR aov.block_num >= _from)
