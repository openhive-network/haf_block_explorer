...
-- hbd_intrest_rate
CREATE OR REPLACE FUNCTION hafbe_backend.get_witness_hbd_interest_rate(_account INT)
...
CREATE OR REPLACE FUNCTION hafbe_backend.get_witness_missed_blocks(_account INT)
RETURNS INT -- noqa: LT01, CP05
LANGUAGE 'plpgsql'
STABLE
AS
...
BEGIN
RETURN (
SELECT count(*) FROM hive.account_operations aov 
WHERE aov.account_id = _account
and aov.op_type_id = 86
);
