...
CREATE OR REPLACE FUNCTION hafbe_backend.get_set_of_witness_voters(_witness_id INT, _order_by TEXT, _order_is TEXT, _limit INT)
...
SELECT
  ...
  COALESCE(todays_votes.votes_daily_change, 0),
  (SELECT hive.get_vesting_balance((SELECT * FROM get_block_num), COALESCE(todays_votes.votes_daily_change, 0)))::BIGINT, 
  COALESCE(all_votes.voters_num, 0),
  COALESCE(todays_votes.voters_num_daily_change, 0),
  ls.price_feed, 
  ls.bias, 
  ls.feed_age, 
  ls.block_size, 
