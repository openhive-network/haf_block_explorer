...
BEGIN
  ...
  SELECT
    ...
    op.timestamp AS _time
  FROM hive.hafbe_app_accounts_view
  WHERE name = account.account_to_recover
  ON CONFLICT ON CONSTRAINT pk_account_parameters
  DO UPDATE SET
...
CREATE OR REPLACE FUNCTION hafbe_app.process_op_c(op RECORD, _comment hive.comment_operation)
RETURNS VOID
LANGUAGE 'plpgsql' VOLATILE
AS
$$
...
CREATE OR REPLACE FUNCTION hafbe_app.process_op_c(op RECORD, vote hive.effective_comment_vote_operation)
RETURNS VOID
LANGUAGE 'plpgsql' VOLATILE
AS
$$
...
BEGIN
  UPDATE hafbe_app.current_witnesses cw SET url = ops.url FROM (
    ...
    FROM (
      SELECT
        trim(both '"' FROM prop.value::TEXT) AS url, op.witness
      FROM (
        SELECT name,value FROM unnest(props.props) AS props
        WHERE name = 'url'
      ) prop
    ) p
  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
        ...
        FROM (
          SELECT trim(both '"' FROM value::TEXT)::JSON AS exchange_rate
          FROM (
            SELECT name,value FROM unnest(props.props) AS props
            WHERE name = 'hbd_exchange_rate'
          ) prop
        ) AS ex
  ...
  UPDATE hafbe_app.current_witnesses cw SET block_size = ops.block_size FROM (
    ...
    FROM (
      SELECT
        block_size::INT
      FROM (
        SELECT trim(both '"' FROM prop.value::TEXT) AS block_size
        FROM (
          SELECT name,value FROM unnest(props.props) AS props
          WHERE name = 'maximum_block_size'
        ) prop
      ) sp
...
BEGIN
  ...
  SET signing_key = o.signing_key FROM (
    ...
    FROM (
      ...
      FROM (
        SELECT COALESCE(
          (SELECT trim(both '"' FROM prop.value::TEXT)
          FROM (
            SELECT name,value FROM unnest(props.props) AS props
            WHERE name = 'new_signing_key'
          ) AS prop),
          (SELECT trim(both '"' FROM prop.value::TEXT) AS signing_key
          FROM (
            SELECT name,value FROM unnest(props.props) AS props
            WHERE name = 'key'
          ) AS prop)
