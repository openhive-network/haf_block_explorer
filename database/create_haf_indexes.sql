...
DO $$
  BEGIN
    IF EXISTS(SELECT 1 FROM pg_index WHERE NOT indisvalid AND indexrelid = (SELECT oid FROM pg_class WHERE relname = 'hive_operations_reversible_permlink_author')) THEN
      RAISE NOTICE 'Dropping invalid index hive_operations_reversible_permlink_author, it will be recreated';
      DROP INDEX hive_operations_reversible_permlink_author;
    END IF;
  END
$$;
...
DO $$
  BEGIN
    IF EXISTS(SELECT 1 FROM pg_index WHERE NOT indisvalid AND indexrelid = (SELECT oid FROM pg_class WHERE relname = 'hive_operations_reversible_delete_permlink_author')) THEN
      RAISE NOTICE 'Dropping invalid index hive_operations_reversible_delete_permlink_author, it will be recreated';
      DROP INDEX hive_operations_reversible_delete_permlink_author;
    END IF;
  END
$$;
