...
The `get_hbd_status/` mainnet suite has 13 Tavern requests covering granularities,
ranges, ordering, defaults, explicit nulls, and invalid inputs. Its `.pat.json`
files originate from the responses saved by CI job `3270133`. Positive patterns
have been adapted to the current response format: supply amounts are strings
and the numeric debt ratio is rounded to three decimal places.
